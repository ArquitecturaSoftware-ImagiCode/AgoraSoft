...
export class ComprasComponent implements OnInit {
 ...
 async procesarCompra() {
  ...
  try {
    ...
    const productosPorProveedor = new Map<string, ProductoSeleccionado[]>();
    this.productosSeleccionados.forEach(item => {
      const proveedorId = item.producto.usuarioProveedor.id!;
    ...
    for (const [proveedorId, items] of productosPorProveedor) {
      const proveedorCompleto = items[0].producto.usuarioProveedor;
